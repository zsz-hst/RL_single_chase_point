...
class ArticulatedPart( object ):
    """ articulated parts for movable parts and a combination of moveable/attached parts of an entity. Section 6.2.94.2"""

    def __init__(self, ParamChangeIndicator, AttachID, C, M, Value):
        """ Initializer for ArticulatedParts"""
        self.ParamChangeIndicator = ParamChangeIndicator
        """ the identification of the Variable Parameter record. Enumeration from EBV"""
    ...
    def serialize(self, outputStream):
        """serialize the class """
        outputStream.write_unsigned_byte(self.ParamChangeIndicator);
        outputStream.write_unsigned_byte(self.AttachID);
        outputStream.write_unsigned_short(self.C);
        outputStream.write_unsigned_int(self.M);
        outputStream.write_long(self.Value);


    def parse(self, inputStream):
        """"Parse a message. This may recursively call embedded objects."""

        self.ParamChangeIndicator = inputStream.read_unsigned_byte();
        self.AttachID = inputStream.read_unsigned_byte();
        self.C = inputStream.read_unsigned_short();
        self.M = inputStream.read_unsigned_int();
        self.Value = inputStream.read_long();
...
class Association( object ):
    """An entity's associations with other entities\
     and/or locations. For each association, this\
      record shall specify the type of the association, \
      the associated entity's EntityID and/or the assoc\
      iated location's world coordinates. This record may\
       be used (optionally) in a transfer transaction to\
        send internal state data from the divesting \
        simulation to the acquiring simulation \
        (see 5.9.4). This record may also be used for \
        other purposes. Section 6.2.9"""
...
class EntityType(  QEvent ):
    """Identifies the type of Entity"""

    def __init__(self, data=None):
        """ Initializer for EntityType"""
        super(EntityType, self).__init__(1000)
        self.entityKind = 0
        """ Kind of entity"""
        self.domain = 0
        """ Domain of entity (air, surface, subsurface, space, etc)"""
...
class FourDouble( object ):
    ...
    def serialize(self, outputStream):
        """serialize the class """
        outputStream.write_double(self.w);
        outputStream.write_double(self.x);
        outputStream.write_double(self.y);
        outputStream.write_double(self.z);
...
class mypdu( EntityInformationFamilyPdu ):
    def __init__(self):
        super(mypdu, self).__init__()
        self.name = []
        self.len = 0
        self.pduType = 112
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super( mypdu, self ).serialize(outputStream)
        outputStream.write_short(self.len);
        for idx in range(self.len):
            #传送中英文，但是KDIS那里没有改
            # outputStream.write_char( self.name[ idx ] );
            #只传送英文0
            outputStream.write_short_char( self.name[ idx ] );
    def parse(self, inputStream):
        super( mypdu, self).parse(inputStream)
        self.len = inputStream.read_short();

        for idx in range(self.len):
            #传送中英文，但是KDIS那里没有改
            self.name.append( inputStream.read_unsigned_char( ))
            #只传送英文
            # self.name.append(inputStream.read_unsigned_char())
            # outputStream.write_byte(self.name[idx]);

class timepointPDU( EntityInformationFamilyPdu ):
    def __init__(self):
        super(timepointPDU, self).__init__()
        self.name = []
        self.len = 0
        self.pduType = 110
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super( timepointPDU, self ).serialize(outputStream)
        outputStream.write_short(self.len);
        for idx in range(self.len):
            outputStream.write_short_char( self.name[ idx ] );
    def parse(self, inputStream):
        super( timepointPDU, self).parse(inputStream)
        self.len = inputStream.read_short();
...
class routePDU( EntityInformationFamilyPdu ):
    def __init__(self):
        super(routePDU, self).__init__()
        self.name = []
        self.len = 0
        self.pduType = 110
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super( routePDU, self ).serialize(outputStream)
        outputStream.write_short(self.len);
        for idx in range(self.len):
            outputStream.write_short_char( self.name[ idx ] );
    def parse(self, inputStream):
        super( routePDU, self).parse(inputStream)
        self.len = inputStream.read_short();
...
class FdtInfoPDU(Pdu):
    def __init__(self):
        super().__init__()
        self.syst = 0.0
        #parse nkeys和lvalues时:需要指定长度
        self.n = 0
        #parse keys时:需要指定长度
        self.nkeys = []
        self.keys = []
        #parse values时:需要指定长度
        self.arrlens = []
        self.arrays = []
        #parse times时:需要指定长度
        self.timeslens = []
    ...
    def serialize(self, outputStream):
        ...
        outputStream.write_double(self.syst)
        outputStream.write_int(self.n)
        for nkey in self.nkeys:
            outputStream.write_int(nkey)
        for key in self.keys:
            outputStream.write(key.encode())
        for arrlen in self.arrlens:
            outputStream.write_int(arrlen)
        for array in self.arrays:
            #value: array
            outputStream.write(array.tobytes())
        for timeslen in self.timeslens:
            outputStream.write_int(timeslen)
        for times in self.times:
            #value: array
            outputStream.write(times.tobytes())
        for borntime in self.borntimes:
            outputStream.write_float(borntime)


    def __call__(self, data):
        self.keys = data.keys()
        self.arrays = [data[key]['array'] for key in self.keys]
        self.times = [data[key]['timestamps'] for key in self.keys]
        self.borntimes = [data[key]['borntime'] for key in self.keys]
        ...
        for array in self.arrays:
            arrlen = len(array)
            self.arrlens.append(arrlen)
        for times in self.times:
            timeslen = len(times)
            self.timeslens.append(timeslen)
...
class HostInfoPdu(Pdu):
    def __init__(self):
        ...
        self.weatherinfo = WeatherState()
        self.pduType = 102

    def serialize(self, outputStream):
        super().serialize(outputStream)
        outputStream.write_float(self.lat)
        outputStream.write_float(self.lon)
        outputStream.write_float(self.alt)
        outputStream.write_float(self.cas)
        outputStream.write_float(self.hdg)
        outputStream.write_float(self.pitch)
        outputStream.write_float(self.bank)
        self.timeinfo.serialize(outputStream)
        self.weatherinfo.serialize(outputStream)
    def parse(self, inputStream):
        super().parse(inputStream)
        self.lat = inputStream.read_float();
        self.lon = inputStream.read_float();
        self.alt = inputStream.read_float();
        self.cas = inputStream.read_float();
        self.hdg = inputStream.read_float();
        self.pitch = inputStream.read_float();
        self.bank = inputStream.read_float();
        self.timeinfo.parse(inputStream)
        self.weatherinfo.parse(inputStream)
...
class TimeInfo(Pdu):
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super().serialize(outputStream)
        outputStream.write_unsigned_int(self.year);
        outputStream.write_unsigned_int(self.month);
        outputStream.write_unsigned_int(self.day);
        outputStream.write_unsigned_int(self.hour);
        outputStream.write_unsigned_int(self.minute);
        outputStream.write_unsigned_int(self.second);
        # outputStream.write_unsigned_int(self.pduType);

    def parse(self, inputStream):
        """"Parse a message. This may recursively call embedded objects."""
        super().parse(inputStream)
        self.year = inputStream.read_unsigned_int();
        self.month = inputStream.read_unsigned_int();
        self.day = inputStream.read_unsigned_int();
        self.hour = inputStream.read_unsigned_int();
        self.minute = inputStream.read_unsigned_int();
        self.second = inputStream.read_unsigned_int();
        # self.pduType = inputStream.read_unsigned_int();


class WeatherState(Pdu):
    def __init__(self):
        super().__init__()
        self.enableRain = 0
        self.enableSnow = 15
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super().serialize(outputStream)
        outputStream.write_unsigned_int(self.enableRain);
        outputStream.write_unsigned_int(self.enableSnow);
        outputStream.write_unsigned_int(self.season);
        outputStream.write_unsigned_int(self.visibility);
        outputStream.write_unsigned_int(self.windSpeed);
        outputStream.write_unsigned_int(self.windDir);
        # outputStream.write_unsigned_int(self.pduType);
    ...
    def parse(self, inputStream):
        ...
        super().parse(inputStream)
        self.enableRain = inputStream.read_unsigned_int();
        self.enableSnow = inputStream.read_unsigned_int();
        self.season = inputStream.read_unsigned_int();
        self.visibility = inputStream.read_unsigned_int();
        self.windSpeed = inputStream.read_unsigned_int();
        self.windDir = inputStream.read_unsigned_int();
        # self.pduType = inputStream.read_unsigned_int();


class TimeInfoPDU(Pdu):
    def __init__(self):
        super(TimeInfoPDU, self).__init__()
        self.year = 0
        self.month = 0
        self.day = 0
        self.hour = 0
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super(TimeInfoPDU, self).serialize(outputStream)
        outputStream.write_unsigned_int(self.year);
        outputStream.write_unsigned_int(self.month);
        outputStream.write_unsigned_int(self.day);
        outputStream.write_unsigned_int(self.hour);
    ...
    def parse(self, inputStream):
        """"Parse a message. This may recursively call embedded objects."""
        super(TimeInfoPDU, self).parse(inputStream)
        self.year = inputStream.read_unsigned_int();
        self.month = inputStream.read_unsigned_int();
        self.day = inputStream.read_unsigned_int();
        self.hour = inputStream.read_unsigned_int();
...
class WeatherStatePdu(Pdu):
    def __init__(self):
        super(WeatherStatePdu, self).__init__()
        self.enableRain = 'b'
        self.enableSnow = 0
        self.season = 0
        self.visibility = 20000
        self.windSpeed = 0
        self.windDir = 0
        self.pduType = 199
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super(WeatherStatePdu, self).serialize(outputStream)
        outputStream.write_unsigned_byte(self.enableRain);
        outputStream.write_unsigned_byte(self.enableSnow);
        outputStream.write_unsigned_byte(self.season);
        outputStream.write_unsigned_int(self.visibility);
        outputStream.write_unsigned_int(self.windSpeed);
        outputStream.write_unsigned_int(self.windDir);
    ...
    def parse(self, inputStream):
        ...
        super(WeatherStatePdu, self).parse(inputStream)
        self.enableRain = inputStream.read_unsigned_char();
        self.enableSnow = inputStream.read_unsigned_byte();
        self.season = inputStream.read_unsigned_byte();
        self.visibility = inputStream.read_unsigned_int();
        self.windSpeed = inputStream.read_unsigned_int();
        self.windDir = inputStream.read_unsigned_int();


class ObserverPdu(Pdu):
    def __init__(self):
        super(ObserverPdu, self).__init__()
        self.observerType = 0
        self.entityID = 0
        self.ICAO = ['Z', 'U', 'U', 'U']
        self.positionLLA = Vector3Double()
        self.orientationPBH = Vector3Double()
        self.relativeXYZ = Vector3Double()
        self.relativePBH = Vector3Double()
        self.pduType = 198

    def serialize(self, outputStream):
        """serialize the class """
        super(ObserverPdu, self).serialize(outputStream)
        for idx in range(0, 4):
            outputStream.write_byte(self.ICAO[idx]);
        outputStream.write_unsigned_byte(self.observerType);
        outputStream.write_unsigned_int(self.entityID);
        self.positionLLA.serialize(outputStream)
        self.orientationPBH.serialize(outputStream)
        self.relativeXYZ.serialize(outputStream)
        self.relativePBH.serialize(outputStream)

    def parse(self, inputStream):
        """"Parse a message. This may recursively call embedded objects."""

        super(ObserverPdu, self).parse(inputStream)
        for idx in range(0, 4):
            inputStream.read_byte(self.ICAO[idx]);
        self.observerType = inputStream.read_unsigned_byte();
        self.entityID = inputStream.read_unsigned_int();
        self.positionLLA.parse(inputStream)
        self.orientationPBH.parse(inputStream)
        self.relativeXYZ.parse(inputStream)
        self.relativePBH.parse(inputStream)


class DrawRadarPDU(Pdu):
    def __init__(self):
        super(DrawRadarPDU, self).__init__()
        self.nID = 0
        self.nState = 0
        self.dLon = 0
        self.dLat = 0
    ...
    def serialize(self, outputStream):
        """serialize the class """
        super(DrawRadarPDU, self).serialize(outputStream)
        outputStream.write_int(self.nID);
        outputStream.write_int(self.nState);
        outputStream.write_double(self.dLon);
        outputStream.write_double(self.dLat);
        outputStream.write_double(self.dAlt);
        outputStream.write_double(self.dSize);

    def parse(self, inputStream):
        """"Parse a message. This may recursively call embedded objects."""
        super(DrawRadarPDU, self).parse(inputStream)
        self.nID = inputStream.read_unsigned_int();
        self.nState = inputStream.read_unsigned_int();
        self.dLon = inputStream.read_double();
        self.dLat = inputStream.read_double();
        self.dAlt = inputStream.read_double();
        self.dSize = inputStream.read_double();
