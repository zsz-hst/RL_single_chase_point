...
def get_trainers(env, num_adversaries, obs_shape_n, action_shape_n, arglist):
    ...
    if arglist.restore or arglist.display: # restore the model
        for i in range(env.n):
            a = arglist.save_dir+'simple_tag/'+'a_c_{}.pt'.format(i)
            actors_cur[i] = torch.load(arglist.save_dir+'simple_tag/'+'a_c_{}.pt'.format(i))
            critics_cur[i] = torch.load(arglist.save_dir+'simple_tag/'+'c_c_{}.pt'.format(i))
            actors_tar[i] = torch.load(arglist.save_dir+'simple_tag/'+'a_t_{}.pt'.format(i))
            critics_tar[i] = torch.load(arglist.save_dir+'simple_tag/'+'c_t_{}.pt'.format(i))
...
def agents_train(arglist, game_step, update_cnt, memory, obs_size, action_size, \
    ...
    if game_step > arglist.learning_start_step and \
        ...
        if update_cnt > arglist.start_save_model and update_cnt % arglist.fre4save_model == 0:
            ...
            model_file_dir = os.path.join(arglist.save_dir, '{}'.format( \
...
def train(arglist):
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        if game_step > 1 and game_step % 100 == 0:   
            mean_agents_r = [round(np.mean(agent_rewards[idx][-200:-1]), 14) for idx in range(env.n)]
            mean_ep_r = round(np.mean(episode_rewards[-200:-1]), 14)
        ...
        for episode_cnt in range(arglist.per_episode_max_len):
            ...
            if arglist.display == True:
                x1.append(new_obs_n[0][0])
                y1.append(new_obs_n[0][1])
                x2.append(new_obs_n[1][0])
                y2.append(new_obs_n[1][1])
                x3.append(new_obs_n[2][0])
                y3.append(new_obs_n[2][1])
                x4.append(new_obs_n[3][0])
                y4.append(new_obs_n[3][1])

            #print('rew:',rew_n[0],'   ',rew_n[1],"   ",rew_n[2],"   ",rew_n[3],"   ")
            #print('1:  ',new_obs_n[0][0],' ',new_obs_n[0][1],'  2:  ',new_obs_n[1][0],' ',new_obs_n[1][1],'  3:  ',new_obs_n[2][0],' ',new_obs_n[2][1],'  4:  ',new_obs_n[3][0],' ',new_obs_n[3][1],)
            #print(episode_cnt,"   ",new_obs_n[0][2]*360,'   ',new_obs_n[0][5]*360,"   ",new_obs_n[0][8]*360,"   ",new_obs_n[0][11]*360,"   ")
            # save the experience
